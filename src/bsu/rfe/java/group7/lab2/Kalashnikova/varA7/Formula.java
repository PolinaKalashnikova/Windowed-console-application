package bsu.rfe.java.group7.lab2.Kalashnikova.varA7;
...
public class Formula extends JFrame {
    ...
    public Double formula1(Double x, Double y, Double z) {
        return Math.pow(Math.log(Math.pow((1+z), 2)) + Math.cos(Math.PI*y*y*y), 1/4) / Math.pow(Math.cos(Math.pow(Math.E, x)) + Math.pow(1/x, 1/2) + Math.pow(Math.E, x*x), Math.sin(x));
    }


    public Double formula2(Double x, Double y, Double z) {
        return Math.pow(Math.sin(Math.pow(z,y)), 2)/Math.pow(1+x*x*x, 1/2);
    }
